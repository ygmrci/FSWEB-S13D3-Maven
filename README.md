...
### org.example.Person Sınıfı

* org.example paketi altında ```org.example.Person``` isimli bir sınıf tanımlayınız.
* Sınıf için 3 tane ```instance variable``` tanımlayınız. 
* ```firstname``` String olmalı, ```lastName``` String olmalı ```age``` int olmalı.
* Kendiniz org.example.Person isimli bir sınıfta olmasını isteyeceğiniz 3 adet daha ```instance variable``` ekleyiniz.
...
* org.example.Person sınıfının içerisinde 4 adet method tanımlayınız.
...
* org.example.Person sınıfından birden fazla person ```instance``` oluşturarak. Bir sınıftan pek çok obje oluşturabilmeyi deneyimleyiniz.

org.example.Person person = new org.example.Person("John", "Doe", 20);
